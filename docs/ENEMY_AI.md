...
## Phase X: Footstep Audio & Proximal Awareness (NEW)

> **Source**: [FOOTSTEPS_IDEAS.md](./FOOTSTEPS_IDEAS.md) — integrated into enemy AI
> **Philosophy**: Augment existing awareness system with audio stimulus — enemies only hear nearby player footsteps

### X.1 Footstep Event System

Every player step emits a footstep event that drives audio, visual effects, and AI perception.

```javascript
// Footstep event structure
var footstep = {
  x: 0,           // grid X
  y: 0,            // grid Y
  foot: 'left' | 'right',
  direction: 0..7, // 8-directional
  loudness: 0,    // 0-10 based on movement type
  timestamp: 0,
  floorId: ''      // for floor-based multipliers
};
```

**Generated by**: Movement cadence clock in `gone-rogue-mobile.js`

**Drives**:
- Audio playback (proximal only)
- Ground effect spawn (dust, footprint)
- Noise grid deposit (AI stimulus)

### X.2 Proximal Audio Rendering (3D 8-Axis)

Enemies only hear player footsteps within their hearing range — not from across the map.

```
AUDIO PERCEPTION RULES:
───────────────────────
Walk:    3 tile radius
Run:     6 tile radius  
Sprint:  8 tile radius

Distance falloff: volume / (distance + 1)

Walls block sound: noise grid propagation stops at wall tiles
```

**Implementation**: In `gone-rogue.js` → `_updateEnemyAwareness()`:

```javascript
function _getPlayerAudioStimulus(enemy, footstep) {
  var dx = enemy.x - footstep.x;
  var dy = enemy.y - footstep.y;
  var dist = Math.sqrt(dx*dx + dy*dy);
  
  // Check wall occlusion
  if (_isWallBetween(enemy.x, enemy.y, footstep.x, footstep.y)) {
    return 0;
  }
  
  // Get loudness based on movement type
  var maxRange = footstep.loudness === 'sprint' ? 8 : 
                 footstep.loudness === 'run' ? 6 : 3;
  
  if (dist > maxRange) return 0;
  
  // Inverse square falloff
  return footstep.loudness / (dist + 1);
}
```

### X.3 Sound Scent Grid (Noise Field)

Instead of NPCs tracking footprints individually, footsteps deposit noise energy into a grid that NPCs follow like a gradient.
...
Decay: -0.5 per frame (footsteps fade after ~3 seconds)

Wall blocking: propagation stops at wall tiles
```

**Grid structure**:
```javascript
var noiseGrid = {};  // key: "x,y", value: { loudness, direction, timestamp }
```

**Update loop** (every 2-3 frames for performance):
```javascript
function updateNoiseGrid() {
  for (var key in noiseGrid) {
    var cell = noiseGrid[key];
    cell.loudness -= 0.5;  // decay
    if (cell.loudness <= 0) delete noiseGrid[key];
  }
}
```

### X.4 Enemy Hearing Behavior

NPCs use the noise grid to investigate disturbances.

```
SUSPICION BEHAVIOR LADDER:
─────────────────────────
0: IDLE      — normal patrol
1: CURIOUS   — heard faint noise (loudness 1-3)
2: INVESTIGATE — heard moderate noise (4-6)
3: ALERT     — heard loud noise (7-9) or saw player
4: CHASE     — confirmed player location

Movement rule: Move to neighbor with highest noise
```

**Implementation** in `gone-rogue.js`:

```javascript
function _updateEnemyHearing(enemy) {
  // Find highest noise in neighboring tiles
  var bestDir = null;
  var bestNoise = 0;
  
  for (var dir = 0; dir < 8; dir++) {
    var nx = enemy.x + _DIR_VECS[dir].x;
    var ny = enemy.y + _DIR_VECS[dir].y;
    var key = nx + ',' + ny;
    var noise = noiseGrid[key] || 0;
  ...
  // Threshold: only investigate if noise exceeds hearing threshold
  if (bestNoise >= 3) {
    enemy.hearingAwareness = Math.min(4, enemy.hearingAwareness + 1);
    enemy.investigateDir = bestDir;
  }
}
```

### X.5 Direction Prediction (Smart Pursuit)

NPCs predict where the player is heading, not just where they've been.

**Implementation**:
```javascript
// Each noise cell stores direction
noiseGrid[key] = { loudness: 7, direction: 2 };  // 2 = East

// When NPC reaches a noise tile, look ahead
function _predictPosition(tile, direction) {
  var vec = _DIR_VECS[direction];
  return {
    x: tile.x + vec.x,
    y: tile.y + vec.y
  };
...
// Prediction strength based on suspicion level
var predictionStrength = {
  1: 0.2,  // curious - weak prediction
  2: 0.5,  // investigate - moderate
  3: 0.8   // alert - aggressive prediction
};
```

### X.6 Ghost Silhouette Feedback (Visual Tracking Indicator)

Players see glowing "ghost footsteps" when enemies are tracking their path — this communicates stealth state without UI.

```
GHOST STEP STATES:
─────────────────
fresh:    0-0.8s  — normal dust/footprint
cooling:  0.8-2.5s — fading
ghost:   2.5-6s   — glowing silhouette (only when NPC tracking)
dead:    remove
```
...
.ghostFootstep {
  ...
  opacity: 0.35;
  animation: ghostStep 220ms forwards;
}
...
**Trigger**: Only animate when NPC actually evaluates that tile in tracking mode.

### X.7 Floor-Based Audio Multipliers

Interior spaces echo more than outdoor tiles.

```
FLOOR AUDIO MULTIPLIERS:
────────────────────────
floor0 (world):    0.9x (gravel/outdoor)
floor0.1 (building): 1.0x (wood)
floor0.1.1 (basement): 1.2x (stone/echo)
```

### X.8 Implementation Tasks

| Task | File | Description |
|------|------|-------------|
| X.1 | `gone-rogue-mobile.js` | Add footstep event emission on step cadence |
| X.2 | `gone-rogue.js` | Add `_getPlayerAudioStimulus()` with wall occlusion |
| X.3 | `gone-rogue.js` | Add noiseGrid with decay propagation |
| X.4 | `gone-rogue.js` | Integrate hearing into `_updateEnemyAwareness()` |
| X.5 | `gone-rogue.js` | Add direction prediction to pursuit logic |
| X.6 | `gone-rogue-canvas.js` | Add ghost silhouette rendering |
| X.7 | `gone-rogue.js` | Floor ID multipliers in audio calculation |
| X.8 | `ground-effects.js` | Reuse ground effect pool for footprints |

### X.9 Cross-System Integration

This system integrates with:

| Existing System | Integration Point |
|----------------|------------------|
| Awareness States (Phase 1) | Hearing adds to SUSPICIOUS/ALERTED |
| Detection Modifiers (1.1) | Audio loudness adds to detection range |
| Alert Cascade (1.2) | ALERTED enemy broadcasts to hearing range |
| Ground Effects | Footprints use existing effect pool |
| Overhead Animator | Show 🔊 when enemy hears player |
...
- [FOOTSTEPS_IDEAS.md](./FOOTSTEPS_IDEAS.md) — Footstep event system, noise grid, ghost silhouettes
- [FOOTSTEP_AUDIO_SYSTEM.md](./FOOTSTEP_AUDIO_SYSTEM.md) — Existing audio infrastructure
...
**Created**: 2026-02-28
**Updated**: 2026-03-09
**Status**: Roadmap — Phase X (Footsteps) added
