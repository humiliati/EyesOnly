...
const SplashScreen = (() => {
  ...
  */
  ...
  /* ============================================================
     ...
     Layers:
       1. Deep-field dust: 600+ single-pixel faint twinklers
       2. Mid-field stars: 120 small stars with gentle drift
       3. Bright stars:    45 larger stars with glow halos
       4. Foreground:      15 bright prominent stars
       5. Milky Way band:  diagonal luminous swath with dense clusters
       6. Turing clusters: reaction-diffusion inspired groupings
     ============================================================ */

  var _starfield = {
    layers:  [],        // [{stars, speed, scale, opacity}]
    master:  null,      // { canvas, ctx }
    milkyWay: [],       // dense stars along the Milky Way band
    turingClusters: [], // easter-egg Turing pattern star clusters
    time:    0,
    rafId:   null,
    running: false,
  };

  // Seeded PRNG for reproducible star placement
  function _sfRng(seed) {
    var s = seed | 0;
    return function () {
  ...
  function _initStarfield() {
    ...
    _starfield.layers = layerDefs.map(function (def) {
      var stars = [];
      // Each layer drifts in its own direction (breaks diagonal-snow uniformity)
      var driftX = Math.cos(def.driftAngle) * def.speed;
      var driftY = Math.sin(def.driftAngle) * def.speed;
      for (var i = 0; i < def.count; i++) {
        // Star color temperature: mix of blue-white, warm white, cool blue
        var temp = rng();
        var cr, cg, cb;
        if (temp < 0.15) {
          // Warm gold/orange tint
          cr = 255; cg = 200 + Math.floor(rng() * 40); cb = 150 + Math.floor(rng() * 40);
        } else if (temp < 0.35) {
          // Cool blue tint
          cr = 160 + Math.floor(rng() * 40); cg = 170 + Math.floor(rng() * 40); cb = 240;
        } else {
          // Neutral white with slight blue
          cr = 200 + Math.floor(rng() * 55); cg = 200 + Math.floor(rng() * 50); cb = 220 + Math.floor(rng() * 35);
        }
        stars.push({
          x:          rng(),
          y:          rng(),
    ...
    // --- Milky Way band: dense concentration of tiny stars along a diagonal ---
    // The band runs from upper-left to lower-right with Gaussian spread
    var mwStars = [];
    for (var i = 0; i < 350; i++) {
      // Parametric position along the band (0→1)
      var p = rng();
      // Gaussian-ish spread perpendicular to the band
      var spread = (rng() + rng() + rng()) / 3 - 0.5; // central-limit approx
      var bandWidth = 0.08 + rng() * 0.06;
      // Band angle: ~35 degrees from horizontal
      var angle = 0.6;
    ...
    _starfield.milkyWay = mwStars;

    // --- Turing pattern clusters: reaction-diffusion inspired groupings ---
    // 3-5 small clusters scattered across the field as easter eggs
    var clusters = [];
    ...
    for (var ci = 0; ci < numClusters; ci++) {
      ...
      var cy = 0.15 + rng() * 0.7;
      var clusterStars = [];
      // Reaction-diffusion pattern: spots with inhibitory ring
      // Core dense cluster + scattered halo with a gap between
      var coreCount = 8 + Math.floor(rng() * 12);
      for (var si = 0; si < coreCount; si++) {
        // Core: tight Gaussian cluster
        var angle2 = rng() * Math.PI * 2;
        var dist2 = (rng() * 0.5 + rng() * 0.5) * 0.018;
        clusterStars.push({
      ...
      // Halo ring: sparse stars at a fixed radius (the "activation" ring)
    ...
    _starfield.turingClusters = clusters;
  ...
  function _starfieldTick() {
    ...
    // Second nebula region (offset, slightly warm)
    var grd2 = ctx.createRadialGradient(W * 0.7, H * 0.6, 0, W * 0.7, H * 0.6, Math.max(W, H) * 0.4);
    grd2.addColorStop(0,   'rgba(25, 10, 18, 0.12)');
    grd2.addColorStop(0.5, 'rgba(10, 5, 12, 0.06)');
    grd2.addColorStop(1,   'rgba(0, 0, 0, 0)');
    ctx.fillStyle = grd2;
    ctx.fillRect(0, 0, W, H);

    // --- Milky Way luminous band ---
    // Broad diagonal glow underlying the dense Milky Way stars
    ctx.save();
    ctx.translate(W * 0.5, H * 0.5);
    ctx.rotate(0.55); // ~31 degrees
    var mwGrad = ctx.createLinearGradient(-W * 0.6, 0, W * 0.6, 0);
    mwGrad.addColorStop(0,    'rgba(0, 0, 0, 0)');
    mwGrad.addColorStop(0.2,  'rgba(20, 18, 40, 0.04)');
    mwGrad.addColorStop(0.4,  'rgba(35, 28, 55, 0.08)');
    mwGrad.addColorStop(0.5,  'rgba(40, 32, 60, 0.10)');
    mwGrad.addColorStop(0.6,  'rgba(35, 28, 55, 0.08)');
    mwGrad.addColorStop(0.8,  'rgba(20, 18, 40, 0.04)');
    mwGrad.addColorStop(1,    'rgba(0, 0, 0, 0)');
    ctx.fillStyle = mwGrad;
    ctx.fillRect(-W, -H * 0.08, W * 2, H * 0.16);
    ctx.restore();

    // Very slow cosmic rotation for the deepest layer only
    var cosmicAngle = t * 0.000015;

    // --- Render main star layers ---
    _starfield.layers.forEach(function (layer, li) {
      layer.stars.forEach(function (star) {
        // Each layer has its own drift direction
        var sx = star.x + t * layer.driftX;
        var sy = star.y + t * layer.driftY;

        // Deepest layer gets very subtle rotation
        if (li === 0) {
        ...
        // Twinkling: sinusoidal brightness modulation
        var twinkle = 1;
        if (layer.twinkle) {
          twinkle = 0.5 + 0.5 * Math.sin(t * star.twinkleRate + star.twinklePhase);
          // Occasional scintillation flash
        ...
        if (r > 0.7) {
          ...
          ctx.fillStyle = 'rgba(' + star.cr + ',' + star.cg + ',' + star.cb + ',' + (alpha * 0.06) + ')';
          ctx.fill();
        }

        // Star core
        if (r < 0.5) {
          // Single-pixel rendering for tiny dust stars (much faster than arc)
    ...
    _starfield.milkyWay.forEach(function (star) {
      ...
      var alpha = star.brightness * twinkle * 0.55;

      // Milky Way stars are tiny blue-white points
      ctx.fillStyle = 'rgba(190, 195, 230, ' + alpha + ')';
      ctx.fillRect(Math.round(px), Math.round(py), 1, 1);
    });

    // --- Turing pattern clusters ---
    var clDriftX = t * 0.00008;
    var clDriftY = t * 0.00005;
    _starfield.turingClusters.forEach(function (cluster) {
      cluster.forEach(function (star) {
        ...
        var alpha = star.brightness * twinkle * 0.7;
        var r = star.r;

        // Slightly warmer tone for cluster stars (subtle difference)
        if (r > 0.45) {
          ctx.beginPath();
          ctx.arc(px, py, r * 2.5, 0, Math.PI * 2);
          ctx.fillStyle = 'rgba(200, 185, 220, ' + (alpha * 0.08) + ')';
          ctx.fill();
        }
        ctx.fillStyle = 'rgba(210, 195, 235, ' + alpha + ')';
        if (r < 0.45) {
          ctx.fillRect(Math.round(px), Math.round(py), 1, 1);
        } else {
          ctx.beginPath();
